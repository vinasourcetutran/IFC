    ...
    */
    $.bindControls = function ($fade_object) {
        $($fade_object.settings.nextLink).on('click', function (event) {
            ...
            {
                var tag = $($fade_object.container).children(':first')[0].tagName.toLowerCase();
                var nextItem = '';
                var previousItem = '';
                ...
                $($fade_object.container).data('current_timeout', setTimeout((function () { $.fadeTimeout($fade_object, nextItem, previousItem, false); }), $fade_object.settings.timeout));                
            }
        });
